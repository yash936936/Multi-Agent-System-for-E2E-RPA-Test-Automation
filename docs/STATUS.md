...
- **Live-Windows re-run confirmed the fix, found one more real gap, now closed too.** PyAutoGUI fail-safe crash: confirmed gone, 595 passing. But 2 of 3 original tests still failed with the same symptom — not a new bug, an incomplete fix: those two tests use the `live_page` fixture (correctly headless by default), but their whole point is testing genuine OCR+DOM agreement, which needs headed rendering. Added a second fixture, `headed_live_page`, used only by those two tests — `live_page`'s default behavior is unchanged for everything else. **Still needs one more live-Windows re-run to fully confirm** — this sandbox can't verify a headed browser session itself.
