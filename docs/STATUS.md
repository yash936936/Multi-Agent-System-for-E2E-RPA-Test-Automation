...
> 4. **(New, 2026-07-13)** Implement Roadmap Phase 20 (proposed, not started — see `Roadmap.md` §6 and `TRD.md` §10): Playwright-first element resolution + Scrapling-style DOM self-heal for `agents/vision/locator.py`, headless-render fix for `agents/capability/link_checker.py`'s client-rendered-page gap, structured audit-log taxonomy, skill quality-tracking. Full design backed by `external_repos.md`'s verified research across 18 repos. This pass (docs reorg + research) touched **zero code** — see `decisions.md` D-015.
> 5. **(New, 2026-07-13)** Implement Roadmap Phase 21 (proposed, not started — see `Roadmap.md` §7 and `TRD.md` §11): Automation Anywhere trigger/validate architecture — new `automation_anywhere_adapter.py` (REST/CLI bot trigger + poll) and `playwright_validator.py` (read-only post-run web-state check), reusing the existing `db_adapter`/`file_adapter` for the database/files validation legs. Naturally sequenced after or alongside Phase 20 since both need the same Playwright dependency. This pass touched **zero code** — see `decisions.md` D-016.
> 6. **(New, 2026-07-13)** Phase 21 delivered: `agents/capability/automation_anywhere_adapter.py` (REST + CLI trigger/poll) and `agents/capability/playwright_validator.py` (read-only Web App leg — `contains_text`/`selector_present`/`selector_text` assertions) are both implemented and registered in `orchestrator/capability_adapter.py::default_registry()` under `CapabilityType.AUTOMATION_ANYWHERE` / `CapabilityType.WEB_VALIDATION`. Playwright is an optional dependency (`pip install .[automation_anywhere]`); the validator fails closed with a clear error if it's missing rather than raising `ImportError`. 13 new tests in `tests/test_automation_anywhere.py` (all passing) cover both adapters plus the full trigger-then-validate pattern. **Not yet done:** `RunEngine`-level enforcement that at least one validation leg must independently confirm the bot's reported status before a run is marked passed (TRD §11.6) — see `decisions.md` D-017.
