...
## Where things stand (2026-07-28 update — Phase 5b of the phased debug pass complete)
- **Two real egress-allowlist gaps found and fixed** in `orchestrator/capability_router.py::_extract_egress_host`. S3 (`CapabilityType.CLOUD`) never had its host resolved at all -- `settings.allowed_capability_hosts` silently never restricted S3 traffic. SharePoint's Graph API host was mislabeled in the router's own docstring as "no fixed host" (it's a hardcoded `graph.microsoft.com`, regardless of tenant) and left as an intentional fail-open case that was actually just wrong. Both now resolve deterministically (S3: `s3.<region>.amazonaws.com`; SharePoint: `graph.microsoft.com`) and can be allowlist-restricted like every other adapter.
- **`azure_adapter.py`, `gcp_adapter.py`: clean**, re-verified against `_URL_PARAM_KEYS`/D-050's Azure env-var handling. Coverage gap noted (not a bug): neither has a dedicated test file, nor does `sharepoint_adapter.py` -- only `cloud_adapter.py`/`workflow_adapter.py` are covered today.
- **775 passed** on the full suite under Xvfb (up from 770 at the end of Phase 5a). Same pre-existing Chromium-binary/no-display sandbox baseline (16 failed, 5 errors) -- zero new regressions.
- **Next action:** Phase 5c (the remaining ~14 `agents/capability/` adapters: `email_adapter.py`, `excel_adapter.py`, `pdf_adapter.py`, `chatops_adapter.py`, `defect_tracker_adapter.py`, `automation_anywhere_adapter.py`, `composio_adapter.py`, `performance_adapter.py`, `security_headers_adapter.py`, `accessibility_adapter.py`, `workflow_adapter.py`, `playwright_validator.py`, `fake_adapter.py`, `file_adapter.py`).

## Prior update (2026-07-28 — Phase 5a of the phased debug pass complete)
- **`link_checker.py`: one bug found and fixed.** `scope="nav"` had no effect at all -- `_AnchorExtractor` never tracked nav depth, so it silently returned every link on the page, identical to `scope="all"`. Fixed by adding `_nav_depth`/`in_nav` tracking mirroring the existing footer handling.
- **`api_adapter.py`: one bug found and fixed, plus new test coverage.** `response.elapsed` crashed the *entire* API check with a misleading "HTTP execution error" whenever the transport doesn't fire httpx's real timing hooks -- true for `httpx.MockTransport`, this codebase's own standard test pattern, which is almost certainly why this adapter had zero tests before now. Fixed by degrading `response_time_ms` to `None` on `RuntimeError` instead of failing the check. `tests/test_api_adapter.py` created from scratch (9 tests).
- **`db_adapter.py`, `db_seed_adapter.py`: clean**, re-verified line by line -- both already carry solid hardening from earlier phases (D-017), 25 existing tests still passing.
- **770 passed** on the full suite under Xvfb (up from 761 at the end of Phase 4). Same pre-existing Chromium-binary/no-display sandbox baseline (16 failed, 5 errors) -- zero new regressions.
- **Next action:** Phase 5b (`agents/capability/`'s cloud/infra adapters: `azure_adapter.py`, `gcp_adapter.py`, `cloud_adapter.py`, `sharepoint_adapter.py`) -- likely least-tested per the doc's own note, since they need real cloud credentials to exercise fully.

## Prior update (2026-07-28 — Phases 2–4 of the phased debug pass complete)
...
- **750 passed** on the full non-browser-dependent suite (up from 749 — one new regression test added).
- **Next action:** Phase 2 of the debugging pass — `runtime/hooks/` (the OS/browser hook layer immediately below what Phase 1 covered).


- **`aura explore` removed entirely.** The CLI command (`aura/cli/explore_cmd.py`), `orchestrator/ui_audit_runner.run_exploration()`, the `"explore"` `IntentKind` and `Router._handle_explore()`, and the API's `"full_exploration": true` opt-in (`api/routers/runs.py`) are all gone. `run_ui_audit()` (used by `aura execute --ui-audit`) is untouched — it shares the same underlying `_run_click_audit()` engine explore used, so nothing about `--ui-audit`'s behavior regressed. For unattended zero-spec runs, use `aura execute --prompt "..."` instead (see `README.md`'s "Autonomy modes" section). **Known gap, disclosed rather than silently dropped:** neither the `guided` nor `autonomous` API request modes currently expose the `--scroll-test`/`--ui-audit` sweep at all — that's CLI-only for now.
- **Bug found and fixed — `has_hero`/`has_footer` misclassification:** `agents/vision/dom_extractor.py::to_ui_elements()` band-classified DOM-sourced elements using a viewport-relative `cy` (from `getBoundingClientRect()`, never adjusted for scroll) divided by the *full document* `scrollHeight` — on any page taller than one screen this collapsed every visible element's fraction toward 0, silently misclassifying hero/body elements as `"nav"`. Fixed by adding a document-relative `docY` alongside the existing viewport-relative `cx`/`cy` (the latter kept as-is since Playwright's `mouse.click()` needs viewport space, not document space) and classifying bands off `docY` instead.
- **Bug found and fixed — below-the-fold elements never discovered:** the DOM discovery path only ever inspected whatever was in the current viewport, with no scrolling — a footer on any page taller than one screen was invisible to it. New `agents/vision/dom_extractor.py::to_ui_elements_full_page()` scrolls the document in viewport-height increments, merges results (deduped by `(text, band)` so sticky/fixed headers aren't double-counted), and records each element's `scroll_y` (`UIElement.scroll_y`) so the click-dispatch step can scroll back to the exact position an element was captured at before dispatching a direct-coordinate click.
- **Bug found and fixed — OCR-dispatched clicks landing off-target ("taskbar jump" bug):** `runtime/hooks/browser.py::get_click_point_in_page()` added `outerWidth - innerWidth` (normally the vertical scrollbar's width, on the content area's *right* edge) into the *left*-offset calculation, directly contradicting the function's own documented "no left/right chrome" assumption. This silently shifted every OCR-dispatched click's x-coordinate (amplified by `devicePixelRatio`), enough to miss small targets entirely while `_dispatch_ocr` still reported success. Fixed by dropping that term; added `tests/test_browser_hook.py::test_get_click_point_in_page_does_not_shift_x_by_scrollbar_width` (this function had zero prior test coverage, which is how the bug went unnoticed).
- **Bug found and fixed — `--record-video` silently overriding `.env`:** `aura execute`'s `--record-video` flag unconditionally reassigned `settings.record_video` on every invocation, so simply not passing the flag reset an `AURA_RECORD_VIDEO=true` `.env` default back to `False`. Fixed with the same `None`-sentinel pattern already used for `--continuous-audit`: `--record-video`/`--no-record-video` now only override `settings.record_video` when explicitly passed.
- **749 passed** on the full non-browser-dependent suite after all of the above (same Chromium/no-display sandbox baseline as prior entries — no new failures introduced).
- **Next action:** none from this pass. Open, not urgent: extend the API's `guided`/`autonomous` request schemas to actually expose `scroll_test`/`ui_audit`, closing the CLI/API parity gap called out above.


- **Gap #1:** `api/routers/runs.py` migrated onto `AuraBrain` — the "fourth variant" `docs/AURA_BRAIN_ARCHITECTURE.md` §1 named as unaddressed is now routed through `AuraBrain().handle()` like every other entrypoint. `_new_engine()` removed. `orchestrator/brain/router.py` gained `built_spec`/`run_id` params to support this.
- **Gap #2:** `Policy.discovery_source()`/`ocr_vocab()`/`band_boundaries()` (built since Phase B2, D-071) are now actually load-bearing — `agents/vision/ui_audit.py::audit_screenshot()` sources vocab/band values from a `Policy()` by default. Editing `brain_knowledge/rules/discovery.yaml`/`bands.yaml` now really changes OCR classification behavior.
- **Gap #3:** `brain_knowledge/playbooks/execute.md`, `execute_interactive.md`, `ui_audit.md` written (matching `explore.md`'s existing style), mirroring the corresponding `Router._handle_*` methods. `playbooks/` now matches `docs/AURA_BRAIN_ARCHITECTURE.md`'s documented tree in full.
- **Gap #4:** `brain_knowledge/prompts/planner_system_prompt.txt`, `planner_retry_prompt.txt`, `requirement_grounding_prompt.txt` written. `orchestrator/brain/context.py::BrainKnowledge` now loads them; `agents/planner/prompts.py` and `agents/planner/spec_generator.py::_build_grounded_text` read from `BrainKnowledge` with the original hardcoded strings as fallback. See D-080 for the `planner_retry_prompt.txt` naming caveat (maps to the user template, not a genuinely distinct retry-specific prompt — none exists in current behavior) and the flagged-but-unfixed dead `DIAGNOSIS_*` constants in `prompts.py`.
- **752 passed / 30 failed / 5 errors**, identical to the D-074-era sandbox-only Chromium/no-display baseline — zero regressions across all four gap fixes.
- **Next action:** none from this audit — all four gaps from the `AURA_REARCHITECTURE_PLAN.md`/`AURA_BRAIN_ARCHITECTURE.md` cross-check are closed. Open follow-on items, not urgent: extend `scripts/check_doc_drift.py` to mechanically diff `playbooks/*.md` against router handlers (flagged since B1/B3, still manual); decide on the dead `DIAGNOSIS_*` prompt constants (D-080).
