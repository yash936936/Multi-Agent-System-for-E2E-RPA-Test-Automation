...
## D-081 — `aura explore` removed; three real bugs found and fixed in the same pass
**Decided:** 2026-07-27
**Decision:** Retire the CLI's zero-instruction `aura explore <url>` command entirely, along with every code path unique to it: `aura/cli/explore_cmd.py`, `orchestrator/ui_audit_runner.py::run_exploration()`, the `"explore"` `IntentKind` and `Router._handle_explore()`, and the web API's `"full_exploration": true` opt-in (`api/routers/runs.py::execute_full_exploration_run`). `orchestrator/ui_audit_runner.py::run_ui_audit()` — the engine behind `aura execute --ui-audit` and `aura ui-audit` — is untouched, since both entry points always shared the same underlying `_run_click_audit()`.

**Why:** Requested directly — `execute` (and its flags) is meant to be the one run command going forward, not a second parallel zero-instruction mode with its own CLI surface, Brain intent, and API branch to keep in sync. `execute --prompt "..."` already covers the "no written spec" case; `--ui-audit` already covers the "click everything and check nav/hero/footer" case. The two commands had converged in capability enough that maintaining both wasn't buying anything `execute`'s existing flags didn't already offer.

**Three real, independent bugs were found and fixed while auditing the surrounding code, not part of the removal itself:**
1. **`has_hero`/`has_footer` band misclassification** (`agents/vision/dom_extractor.py::to_ui_elements`) — DOM-sourced elements were band-classified using a viewport-relative `cy` (`getBoundingClientRect()`, never adjusted for scroll) divided by the *full document* `scrollHeight`. On any page taller than one viewport this collapsed every visible element's fraction toward 0, silently misclassifying hero/body elements as `"nav"`. Fixed by adding a document-relative `docY` field and classifying bands off that instead, while keeping `cx`/`cy` viewport-relative (required for Playwright's `mouse.click()`, which operates in viewport space).
2. **Below-the-fold elements never discovered at all** — the DOM discovery path only ever inspected the current viewport, with no scrolling, so a footer on a page taller than one screen was invisible to it regardless of bug #1. New `to_ui_elements_full_page()` scrolls the document in viewport-height increments (capped, same philosophy as `orchestrator/autoscan.py`), merges results deduped by `(text, band)` (so a sticky/fixed-position header isn't counted as a new element at every scroll stop), and records each element's `scroll_y` (`UIElement.scroll_y`) so a later direct-coordinate click can scroll back to the exact position it was captured at before dispatching.
3. **OCR-dispatched clicks landing off-target ("taskbar jump" bug)** — `runtime/hooks/browser.py::get_click_point_in_page()` added `window.outerWidth - window.innerWidth` (normally the vertical scrollbar's width, on the content area's *right* edge) into the *left*-offset calculation, directly contradicting the function's own documented "no left/right chrome" assumption. This silently shifted every OCR-dispatched click's x-coordinate (amplified by `devicePixelRatio`) — enough to miss small targets entirely while `_dispatch_ocr` still reported a successful dispatch, since no exception was ever raised. Fixed by dropping that term. This function had zero prior test coverage (`tests/test_browser_hook.py::test_get_click_point_in_page_does_not_shift_x_by_scrollbar_width` added), which is how the bug went unnoticed.

**Also fixed, found during the same flag audit (unrelated to explore):** `aura execute --record-video` unconditionally reassigned `settings.record_video` on every invocation, so simply omitting the flag reset an `AURA_RECORD_VIDEO=true` `.env` default back to `False`. Fixed with the same `None`-sentinel pattern `--continuous-audit` already used: `--record-video`/`--no-record-video` now only override `settings.record_video` when explicitly passed.

**Verified:** `python -m pytest tests/ -q --ignore=tests/integration` (minus the same Chromium-binary/no-display sandbox-only exclusions every prior entry since D-074 has used) → 749 passed, zero regressions from the pre-removal baseline. `tests/test_explore_cmd.py` and `tests/integration/test_explore_against_fixtures.py` deleted outright (they tested only the removed code); explore-specific test blocks removed from `tests/test_ui_audit_runner.py`, `tests/test_brain.py`, `tests/test_api_service.py`, `tests/test_parallel_execution.py` (docstring only) without touching the surviving tests in those files.

**Known, disclosed gap (not fixed in this pass):** neither `mode: "guided"` nor `mode: "autonomous"` requests to `POST /api/v1/test-runs` currently expose `--scroll-test`/`--ui-audit` at all (`execute_run`/`execute_autonomous_run` in `api/routers/runs.py` don't forward those params to `Router._handle_execute_requirement`). This isn't a regression from removing `full_exploration` — that API opt-in never routed through `execute`'s flags either — but it means the landmark/link-check sweep is CLI-only right now.
